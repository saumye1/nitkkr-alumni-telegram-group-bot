...
module.exports = {
    getmyBatchMates : function(params) {
        return new Promise( (resolve , reject) => {
            params.dbo.collection(config.get('mongoCollections.users')).findOne(params.condition, function(error, user) {
                if(error) {
                    ...
                    dbo.collection(config.get('mongoCollections.users')).find({"batch" : new RegExp(user.batch)}).toArray( function(error , batchmates) {
                        if(error){
                            reject(error);
                        }else {
    ...
    getBatchMatesByYear: function(params) {
        return new Promise( (resolve , reject) => {
